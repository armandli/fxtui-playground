...
// Creates a bordered message block with the username in the border title
// and the message body auto-wrapped to fill the available width.
Element make_message_block(
...
int estimate_height(std::string_view msg, int terminal_width) {
    ...
    int terminal_width)
{
  ...
  while (not messages.empty() and total > available_height) {
...
int main() {
  ...
  std::deque<Message> messages = {
      ...
      {"alice",
          "We went with Rust for the backend and React on the front end. "
          "Took a while but the performance is great."},
      {"dave", "Have you benchmarked it against the old Go service?"},
      {"bob",
          "Yeah, latency is down about 40%. The lifetime borrow checker "
          "was painful at first but pays off."},
      {"you",
          "Did you run into any issues with error propagation across "
          "async boundaries?"},
      {"alice",
          "Tons! anyhow + thiserror saved us. Highly recommend that combo."},
  };
  ...
  auto input_comp = Input(
      &input_text, "Type a message and press Enter…", opt);
