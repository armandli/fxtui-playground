...
void display_calendar(int date_int) {
  ...
  const std::string title =
      std::string(month_name(month)) + " " + std::to_string(year);
  ...
  for (int d = 0; d < 7; ++d)
    dow_cells.push_back(
        text(dow[d]) | bold | center | size(WIDTH, GREATER_THAN, 5) | flex);
    ...
    for (int col = 0; col < 7; ++col) {
      ...
      if (day_num < 1 or day_num > num_days) {
        ...
        row_cells.push_back(
            ...
            size(WIDTH, GREATER_THAN, 5) | flex);
        continue;
      }

      const bool highlight =
          now.year == year and now.month == month and now.day == day_num;

      // 3-char label ("  1"…"  9" or " 10"…" 31") makes text min-width 3, so
      // bordered cell min-width = 3 + 2 borders = 5 for all days.
      const std::string label =
          day_num < 10 ? "  " + std::to_string(day_num)
                        : " " + std::to_string(day_num);
