...
int main() {
  double theta = 0.0;
  constexpr double kStep = 2.0 * std::numbers::pi / 180.0;  // 2 deg per tick
  ...
  auto renderer = Renderer([&] {
    ...
    auto draw_fn = [canvas_w, canvas_h, theta](Canvas& c) {
      ...
      common::Point v0 = rotate(cx, cy, radius, -std::numbers::pi / 2.0, theta);
      common::Point v1 = rotate(cx, cy, radius, std::numbers::pi / 6.0, theta);
      common::Point v2 = rotate(
          cx, cy, radius, 5.0 * std::numbers::pi / 6.0, theta);

      common::fill_triangle(c, v0, v1, v2, Color::Yellow);
    };
  ...
  common::AnimationTimer timer(screen, std::chrono::milliseconds(33));
