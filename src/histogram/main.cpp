...
Element build_x_axis(int canvas_cols, int y_axis_width) {
  ...
  for (auto& [v, label] : ticks) {
    int pos = static_cast<int>(
        (v - kRangeMin) / (kRangeMax - kRangeMin) * canvas_cols);
...
int main() {
  ...
  auto renderer = Renderer([&] {
    ...
    // Overhead: y_axis(kYAxisWidth) + border_left(1) + border_right(1) =
    // kYAxisWidth + 2
    ...
    // Capture canvas dimensions by value — canvas fn is called after
    // Renderer returns
    auto draw_fn = [canvas_w, canvas_h, &counts, max_count, &highlighted](
        Canvas& c) {
      ...
      for (int i = 0; i < kNumBins; ++i) {
        ...
        int bar_h = max_count > 0
            ? static_cast<int>(
                  static_cast<double>(counts[i]) / max_count * canvas_h)
            : 0;
    ...
    // Y-axis: 1-row spacers at top/bottom align labels with canvas content
    // inside border
    auto y_axis = vbox({
        filler() | size(HEIGHT, EQUAL, 1),
        text(std::format("{:>{}}", max_count, kYAxisWidth - 1)) |
            align_right,
        filler(),
    ...
    auto canvas_elem = canvas(canvas_w, canvas_h, draw_fn) |
        reflect(histogram_box) | border | flex;
    ...
    auto status = text(std::format(
        "Bar {:3d} | [{:+.3f}, {:+.3f}] | Count: {:6d} | arrow keys / "
        "click to navigate | q to quit",
        highlighted, lo, hi, counts[highlighted])) | center;
