...
inline ftxui::Element render_glyph_cell(
    ftxui::Color bg,
    ftxui::Color fg,
  ...
  for (int y = 0; y < scale; ++y) {
    if (y == mid_row and not glyph.empty()) {
      int left = (width - 1) / 2;
      int right = width - 1 - left;
      rows.push_back(
          text(std::string(left, ' ') + glyph + std::string(right, ' ')));
    } else {
...
inline ftxui::Element render_grid(
    int rows,
    int cols,
    const std::function<ftxui::Element(int, int)>& cell_fn) {
  using namespace ftxui;
  Elements grid_rows;
...
// Inverse of the render_grid/render_glyph_cell layout: maps a terminal-cell
...
// used to render it. Kept beside compute_grid_scale so the two can never
// drift apart if the cell-size formula ever changes.
inline std::pair<int, int> pixel_to_cell(
    int mx,
    int my,
    const ftxui::Box& box,
    int scale) {
