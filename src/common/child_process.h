...
// signal(SIGINT, common::TerminateChildAndReraise) (and SIGTERM) at the top
// of main(), before constructing a ChildProcess -- this only matters in the
// window before an FTXUI ScreenInteractive::Loop() has installed its own
// signal handling, since that takes over gracefully once running.
inline void TerminateChildAndReraise(int sig) {
...
inline pid_t SpawnChildProcess(const std::string& path, const std::vector<std::string>& args,
                                int& in_write_fd, int& out_read_fd) {
...
class ChildProcess {
public:
    ChildProcess(std::string path, std::vector<std::string> args, std::function<void(const std::string&)> on_line)
        : path_(std::move(path)), args_(std::move(args)), on_line_(std::move(on_line)) {}

    ChildProcess(std::string path, std::function<void(const std::string&)> on_line)
    ...
    bool Start() {
        ...
        pid_ = detail::SpawnChildProcess(path_, args_, in_fd, out_fd);
        if (pid_ < 0)
            return false;
        ...
        reader_thread_ = std::thread(&ChildProcess::ReaderLoop, this);
        return true;
    }
