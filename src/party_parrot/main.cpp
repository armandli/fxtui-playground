...
Color source_pixel_color(int frame, int sx, int sy) {
...
int main() {
  ...
  auto renderer = Renderer([&] {
    ...
    for (int ty = 0; ty < term_rows; ++ty) {
      int py_top = ty * 2;
      int py_bot = ty * 2 + 1;
      bool row_in_top = py_top >= off_y and py_top < off_y + size;
      bool row_in_bot = py_bot >= off_y and py_bot < off_y + size;
      ...
      for (int tx = 0; tx < cols; ++tx) {
        ...
        if (tx >= off_x and tx < off_x + size) {
          int sx = std::clamp(
              (tx - off_x) * kFrameWidth / size,
              0,
              kFrameWidth - 1);
          if (row_in_top) {
            int sy = std::clamp(
                (py_top - off_y) * kFrameHeight / size,
                0,
                kFrameHeight - 1);
            fg = source_pixel_color(frame_index, sx, sy);
          }
          if (row_in_bot) {
            int sy = std::clamp(
                (py_bot - off_y) * kFrameHeight / size,
                0,
                kFrameHeight - 1);
            bg = source_pixel_color(frame_index, sx, sy);
          }
        }
