...
int main() {
  ...
  // Double buffers: front is what the renderer reads; back is what update
  // writes.
  ...
  auto update = [&] {
    ...
    // Swap buffers: the renderer will read the newly painted front next
    // frame.
    std::swap(front, back);
  };
  ...
  common::AnimationTimer timer(screen, std::chrono::milliseconds(100));
