...
std::vector<Point> generate_points() {
  ...
  for (int i = 0; i < kN; ++i) {
    ...
    pts.push_back(
        {3.0 * std::cos(t) + noise(rng), 1.5 * std::sin(t) + noise(rng), 0});
  }
  // Ellipse 1: x=1.5cos(t)+1, y=2.5sin(t)+2, center (1,2)
  for (int i = 0; i < kN; ++i) {
    double t = 2.0 * std::numbers::pi * i / kN;
    pts.push_back(
        {1.5 * std::cos(t) + 1.0 + noise(rng),
...
std::pair<int, int> data_to_canvas(
    double x,
    double y,
    double xmin,
    double xmax,
...
int main() {
  ...
  int selected_idx = -1;
  // Filled by reflect() each frame; zero-init means Contain() returns false
  ...
  auto renderer = Renderer([&] {
    ...
    return vbox({
        text(
            "Scatter Plot — Two Ellipses with Gaussian Noise  "
            "(N=80/series, σ=0.15)")
            | bold | center,
        separator(),
        hbox(main_row) | flex,
  ...
  auto app = CatchEvent(renderer, [&](Event e) -> bool {
    if (common::is_left_click_press(e)) {
      ...
      if (canvas_box.Contain(mx, my)) {
        ...
        for (int i = 0; i < static_cast<int>(points.size()); ++i) {
          auto [px, py] = data_to_canvas(
              points[i].x, points[i].y, xmin, xmax, ymin, ymax, cw, ch);
