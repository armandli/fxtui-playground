...
int main() {
    ...
    for (int r = 0; r < kRows; ++r) {
      for (int c = 0; c < kCols; ++c) {
        if (density(rng) < kInitialDensity) {
          ...
          front.color[idx(r, c)] = random_color(rng);
        }
      }
    }
  }
  ...
  auto update = [&] {
      ...
      for (int r = r0; r < r1; ++r) {
        for (int c = 0; c < kCols; ++c) {
          ...
          hsum[idx(r, c)] = front.alive[idx(r, left)] +
              front.alive[idx(r, c)] + front.alive[idx(r, right)];
        }
      }
    });
    ...
    parallel_for_rows(kRows, [&](int r0, int r1) {
      ...
      std::mt19937 rng{
          std::random_device{}() ^ static_cast<unsigned>(r0 * 2654435761u)};
      for (int r = r0; r < r1; ++r) {
        ...
        for (int c = 0; c < kCols; ++c) {
          ...
          bool now_alive = was_alive ? (neighbors == 2 or neighbors == 3)
                                      : (neighbors == 3);
          back.alive[self_i] = now_alive;
          if (now_alive) {
            back.color[self_i] =
                was_alive ? front.color[self_i] : random_color(rng);
          }
        }
      }
    });
