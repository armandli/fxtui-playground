...
// via the upper-half-block technique (see RenderPieceCell) so a piece scales
// to fill its board cell instead of staying a single fixed-size glyph. Side
// (white/black) is conveyed purely by foreground color, same as PieceGlyph
// in piece.h -- these bitmaps are shape-only, not per-side.
//
...
inline const PieceBitmap& PieceBitmapFor(PieceType t) {
    switch (t) {
        case PieceType::King:   return kKingBitmap;
        case PieceType::Queen:  return kQueenBitmap;
        case PieceType::Rook:   return kRookBitmap;
        case PieceType::Bishop: return kBishopBitmap;
        case PieceType::Knight: return kKnightBitmap;
        case PieceType::Pawn:   return kPawnBitmap;
    }
    return kPawnBitmap;
}

inline bool PieceBitmapPixelOn(const PieceBitmap& bitmap, int x, int y) {
    return (bitmap[y] >> (kPieceBitmapSize - 1 - x)) & 1;
}

// Renders a scale-tall, 2*scale-wide cell (matching RenderGlyphCell's
// footprint exactly, so ComputeGridScale/PixelToCell need no changes) by
...
inline ftxui::Element RenderPieceCell(ftxui::Color bg, ftxui::Color fg, const PieceBitmap& bitmap, int scale) {
    using namespace ftxui;
    Elements rows;
    for (int ty = 0; ty < scale; ++ty) {
        Elements cells;
        for (int tx = 0; tx < 2 * scale; ++tx) {
            int bx = std::clamp(tx * kPieceBitmapSize / (2 * scale), 0, kPieceBitmapSize - 1);
            int by_top = std::clamp(ty * kPieceBitmapSize / scale, 0, kPieceBitmapSize - 1);
            int by_bot = std::clamp((2 * ty + 1) * kPieceBitmapSize / (2 * scale), 0, kPieceBitmapSize - 1);
            Color top = PieceBitmapPixelOn(bitmap, bx, by_top) ? fg : bg;
            Color bottom = PieceBitmapPixelOn(bitmap, bx, by_bot) ? fg : bg;
