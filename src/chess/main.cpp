    ...
    StockfishClient::DifficultyConfig& config)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--elo") {
      int v = 0;
      if (not next_int(v) or v < 1320 or v > 3190) {
        print_usage(std::cerr, argv[0]);
        return 1;
      }
...
// read as a bitmap silhouette, so render_cell falls back to the plain glyph.
...
int display_row(int rank, Side human) {
  return human == Side::White ? 7 - rank : rank;
}
int display_col(int file, Side human) {
  return human == Side::White ? file : 7 - file;
}
...
Element render_cell(
    ...
    bool is_check)
{
  Color bg = is_dark_square(pos) ? kDarkSquare : kLightSquare;
  if (is_check)
    bg = kCheckHighlight;
  else if (is_selected)
  ...
  if (not board[pos.rank][pos.file])
    return common::render_glyph_cell(bg, Color::White, "", scale);

  const Piece& p = *board[pos.rank][pos.file];
  Color fg = (p.side == Side::White) ? kWhitePieceFg : kBlackPieceFg;
  if (scale < kMinBitmapScale)
    return common::render_glyph_cell(bg, fg, piece_glyph(p.type), scale);
  return render_piece_cell(bg, fg, piece_bitmap_for(p.type), scale);
}

Element render_board(
    const Board& board,
    int scale,
    Side human_color,
    const std::optional<Position>& selected,
    const std::vector<Move>& legal,
    const std::optional<Position>& king_in_check)
{
  ...
  return common::render_grid(8, 8, [&](int row, int col) {
    // display_col/display_row are self-inverse, so calling them on a display
    // coordinate recovers the underlying board file/rank.
    Position pos{display_col(col, human_color), display_row(row, human_color)};
    bool is_selected = selected.has_value() and *selected == pos;
    bool is_target =
        std::find(targets.begin(), targets.end(), pos) != targets.end();
    bool is_check = king_in_check.has_value() and *king_in_check == pos;
    return render_cell(board, pos, scale, is_selected, is_target, is_check);
  });
}

int compute_scale(Dimensions term) {
  int avail_rows = std::max(1, term.dimy - 2);  // board border top/bottom
  // border + separator + side panel
  int avail_cols = std::max(1, term.dimx - kSidePanelWidth - 3);
  return common::compute_grid_scale(avail_rows, avail_cols, 8, 8);
}

std::optional<Position> king_in_check_square(
    const Board& board,
    const GameState& state)
{
  Position king = find_king(board, state.side_to_move);
  if (is_square_attacked(board, king, opposite(state.side_to_move)))
    return king;
  return std::nullopt;
}

Element render_promo_choice(PieceType t, Side side) {
  Color fg = (side == Side::White) ? kWhitePieceFg : kBlackPieceFg;
  return text(" " + piece_glyph(t) + " ") | color(fg) |
      bgcolor(Color::RGB(90, 90, 90)) | border;
}

std::string difficulty_text(const StockfishClient::DifficultyConfig& c) {
  if (c.limit_strength)
    return "Difficulty: ~" + std::to_string(c.elo) + " Elo";
  if (c.go_mode == StockfishClient::GoMode::Movetime)
    return "Difficulty: full strength, " + std::to_string(c.go_value) +
        "ms/move";
  return "Difficulty: full strength, depth " + std::to_string(c.go_value);
}

std::string result_text(GameResult r, Side side_to_move) {
  switch (r) {
    case GameResult::Checkmate:
      return std::string(side_to_move == Side::White ? "Black" : "White") +
          " wins by checkmate";

    break; case GameResult::Stalemate:
      return "Draw by stalemate";

    break; case GameResult::DrawFiftyMove:
      return "Draw by fifty-move rule";

    break; case GameResult::DrawThreefold:
      return "Draw by threefold repetition";

    break; case GameResult::DrawInsufficientMaterial:
...
int main(int argc, char** argv) {
  // common::terminate_child_and_reraise is load-bearing only before
  ...
  signal(SIGINT, common::terminate_child_and_reraise);
  signal(SIGTERM, common::terminate_child_and_reraise);

  StockfishClient::DifficultyConfig config;
  if (auto code = parse_args(argc, argv, config))
    return *code;

  ScreenInteractive screen = ScreenInteractive::Fullscreen();
  StockfishClient client("stockfish", [&] { screen.PostEvent(Event::Custom); });
  if (not client.start(config)) {
  ...
  GameState state;
  state.position_history[position_key(board, state)] = 1;
  std::vector<Move> legal = generate_legal_moves(board, state);
  ...
  auto ApplyMoveAndAdvance = [&](const Move& m) {
    ...
    result = compute_game_result(board, state, legal);
    selected.reset();
    if (result != GameResult::Ongoing) {
      phase = GamePhase::GameOver;
      game_over_text = result_text(result, state.side_to_move);
    } else {
      phase = GamePhase::Playing;
      if (state.side_to_move != human_color)
        client.request_best_move(state.moves_uci);
    }
  };
    ...
    if (phase == GamePhase::ChoosingColor) {
      board_or_prompt =
          vbox({
              ...
              hbox({
                  text("  Play White  ") |
                      bgcolor(Color::RGB(240, 240, 240)) | color(Color::Black) |
                      border | reflect(white_choice_box),
                  text("   "),
                  text("  Play Black  ") |
                      bgcolor(Color::RGB(30, 30, 30)) | color(Color::White) |
                      border | reflect(black_choice_box),
              }) | center,
          }) |
          center | flex;
      side_panel = text("") | size(WIDTH, EQUAL, kSidePanelWidth);
    } else {
      int scale = compute_scale(term);
      auto king_check = king_in_check_square(board, state);
      Element backdrop =
          render_board(board, scale, human_color, selected, legal, king_check) |
          reflect(board_box) | border;
      ...
      if (phase == GamePhase::ChoosingPromotion) {
        Element prompt =
            vbox({
                ...
                hbox({
                    render_promo_choice(PieceType::Queen, human_color) |
                        reflect(promo_q_box),
                    render_promo_choice(PieceType::Rook, human_color) |
                        reflect(promo_r_box),
                    render_promo_choice(PieceType::Bishop, human_color) |
                        reflect(promo_b_box),
                    render_promo_choice(PieceType::Knight, human_color) |
                        reflect(promo_n_box),
                }),
            }) |
      ...
      panel_lines.push_back(text(difficulty_text(config)));
      panel_lines.push_back(text(
          std::string("You are: ") +
          (human_color == Side::White ? "White" : "Black")));
      panel_lines.push_back(separator());
      if (phase == GamePhase::GameOver) {
        panel_lines.push_back(text(game_over_text) | bold);
      } else if (client.is_thinking()) {
        panel_lines.push_back(text("Stockfish is thinking...") | bold);
      } else {
        const char* status = (state.side_to_move == human_color)
            ? "Your move"
            : "Waiting for engine...";
        panel_lines.push_back(text(status) | bold);
      }
      panel_lines.push_back(separator());
      ...
      for (size_t i = start_pair * 2; i < state.moves_uci.size(); i += 2) {
        std::string line =
            "  " + std::to_string(i / 2 + 1) + ". " + state.moves_uci[i];
      ...
      side_panel =
          vbox(std::move(panel_lines)) | size(WIDTH, EQUAL, kSidePanelWidth);
    }
  ...
  auto app = CatchEvent(renderer, [&](Event e) -> bool {
    if (e == Event::Custom) {
      if (phase == GamePhase::Playing) {
        if (auto uci = client.take_best_move()) {
          auto it = std::find_if(
              legal.begin(), legal.end(),
              [&](const Move& m) { return uci_move_string(m) == *uci; });
          if (it == legal.end()) {
            phase = GamePhase::GameOver;
    ...
    if (e == Event::Character('q') or e == Event::Character('Q') or
        e == Event::Escape) {
      screen.Exit();
      return true;
    }
    if (e == Event::Character('n') or e == Event::Character('N')) {
    ...
    if (not common::is_left_click_press(e))
      return false;
    ...
    if (phase == GamePhase::ChoosingPromotion) {
      ...
      auto it = std::find_if(legal.begin(), legal.end(), [&](const Move& m) {
        return m.from == pending_from and m.to == pending_to and
            m.promotion == chosen;
      });
      if (it != legal.end())
        ApplyMoveAndAdvance(*it);
    ...
    if (phase != GamePhase::Playing or state.side_to_move != human_color)
      return false;
    if (not board_box.Contain(mx, my))
      return false;
    ...
    // compute_scale() the renderer used instead, anchored at board_box's origin.
    int scale = compute_scale(Terminal::Size());
    auto [click_row, click_col] =
        common::pixel_to_cell(mx, my, board_box, scale);
    if (click_col < 0 or click_col >= 8 or click_row < 0 or click_row >= 8)
      return false;
    Position clicked{
        display_col(click_col, human_color),
        display_row(click_row, human_color)};

    if (not selected) {
      if (board[clicked.rank][clicked.file] and
          board[clicked.rank][clicked.file]->side == human_color)
        selected = clicked;
    } else if (clicked == *selected) {
      selected.reset();
    } else if (board[clicked.rank][clicked.file] and
               board[clicked.rank][clicked.file]->side == human_color) {
      selected = clicked;
    } else {
      std::vector<Move> matches;
      for (auto& m : legal)
        if (m.from == *selected and m.to == clicked)
          matches.push_back(m);
      if (matches.empty()) {
        selected.reset();
