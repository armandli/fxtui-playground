...
struct Move {
  ...
  // Castling is detected structurally (king moving 2 files) -- no explicit
  // flag needed.
};

inline bool operator==(const Move& a, const Move& b) {
  return a.from == b.from and a.to == b.to and a.promotion == b.promotion and
...
struct GameState {
  ...
  std::map<std::string, int> position_history;  // position_key -> occurrence count
};
...
inline std::string uci_move_string(const Move& m) {
  ...
  if (m.promotion) {
    switch (*m.promotion) {
      ...
      break; default:  // no other PieceType values are valid promotions
        ;
    }
  }
  return s;
...
inline bool is_dark_square(Position p) {
