...
inline const Vec3 kLightDir = Normalize(Vec3{-0.5, 0.6, 0.65});
...
inline ftxui::Color ShadeFace(const Vec3& world_normal) {
    double diffuse = std::max(0.0, Dot(world_normal, kLightDir));
    double intensity = std::clamp(kAmbient + (1.0 - kAmbient) * diffuse, 0.0, 1.0);
    return ftxui::Color::Interpolate(static_cast<float>(intensity), kDarkGreen, kBrightGreen);
}
